...
- Q: 当部署失败时,系统应该如何通知团队? → A: GitHub Actions 失败状态 + GitHub Issue 自动创建
- Q: 当多个 PR 同时合并到 master 触发多个部署时,系统应该如何处理? → A: 串行执行,使用队列机制,后续部署等待前一个完成
- Q: 部署后的健康检查应该验证哪些内容? → A: HTTP /health 端点 + 返回 JSON 格式验证 + 服务活跃状态
- Q: 旧的部署版本应该保留多久? → A: 保留最近 3 个版本 + 自动清理 7 天前的版本
- Q: 当测试在本地通过但在 CI 环境中失败时,应该如何处理? → A: 提供本地 CI 环境复现工具(如 act)+ 环境差异文档
...
- 当部署到阿里云 ECS 中途失败时会发生什么(网络超时、服务器不可用)? → 系统维持之前版本并通过 GitHub Issue 通知团队
- 系统如何处理并发合并到 master(同时触发多个部署)? → 使用 GitHub Actions concurrency 配置实现串行队列执行
- 如果测试在本地通过但由于环境差异在 CI 环境中失败会发生什么? → 提供 act 工具和环境差异文档帮助开发者本地复现
- 系统如何安全处理秘密和凭证(阿里云访问密钥、SSH 密钥)? → 使用 GitHub Secrets 安全存储,禁止在日志中暴露
- 当阿里云 ECS 服务器在部署期间磁盘空间或内存不足时会发生什么? → 部署前检查资源,失败时触发自动清理机制
- 如果部署版本在部署后发现关键错误,系统如何处理回滚? → 维持符号链接到最近 3 个版本,支持快速手动回滚
- 旧版本如何管理? → 保留最近 3 个版本,自动清理 7 天前的版本以控制磁盘使用
...
- **FR-003**: 系统必须运行完整的测试套件(单元测试、集成测试、契约测试)并生成覆盖率报告,强制执行最低 80% 覆盖率
- **FR-004**: 系统必须在任何自动化检查失败时阻止 Pull Request 合并
- **FR-005**: 系统必须在代码合并到 master 分支时自动触发部署
- **FR-006**: 系统必须安全连接到阿里云 ECS 服务器进行部署
- **FR-007**: 系统必须在部署完成后通过 HTTP /health 端点验证应用程序健康状态,包括 JSON 格式验证和服务活跃状态检查
- **FR-008**: 系统必须在部署失败时维持之前的版本(回滚能力),保留最近 3 个版本并支持快速回滚
- **FR-009**: 系统必须为测试和部署进度提供可见的状态指示器
- **FR-010**: 系统必须记录所有部署活动以供审计和调试
- **FR-011**: 系统必须安全处理部署凭证,不在日志或代码中暴露
- **FR-012**: 系统必须在部署失败时通过 GitHub Actions 失败状态 + 自动创建 GitHub Issue 的方式通知团队成员
- **FR-013**: 系统必须使用 concurrency 配置确保并发部署请求串行执行,避免状态冲突
- **FR-014**: 系统必须自动清理 7 天前的旧部署版本,同时保留最近 3 个版本
- **FR-015**: 系统必须提供本地 CI 环境复现工具(act)和环境差异文档,帮助开发者调试 CI 失败
...
- **SC-009**: 代码测试覆盖率保持在 80% 以上(符合宪法第 V 条可测试性要求)
...
- 自动化部署到单个阿里云 ECS 实例
- 部署后健康检查验证(HTTP /health + JSON 验证 + 服务状态)
- 基于符号链接的回滚机制(保留最近 3 个版本)
- 使用 GitHub Secrets 的安全凭证管理
- GitHub Actions 界面中的部署状态可见性
- 部署失败时自动创建 GitHub Issue 通知
- Concurrency 配置确保串行部署执行
- 自动版本清理(7 天前的版本)
- 本地 CI 环境复现工具(act)和环境差异文档
