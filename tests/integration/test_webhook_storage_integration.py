...
class TestWebhookStorageIntegration:
    ...
    def test_webhook_persists_json_message_to_jsonl(self, client, temp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr(webhook_handler, "get_jsonl_writer", lambda: test_writer)
    ...
    def test_webhook_handles_storage_failure_gracefully(self, client, monkeypatch):
        ...
        def failing_writer():
            ...
            writer = JSONLWriter(base_dir="/invalid/path/that/does/not/exist")
            return writer

        monkeypatch.setattr(webhook_handler, "_jsonl_writer", None)
        monkeypatch.setattr(webhook_handler, "get_jsonl_writer", failing_writer)
