...
| 接口名称 | 端点 | 说明 |
|---------|------|------|
| 恢复实例 | `/client/restore` | 恢复已停止的实例 |
| 停止实例 | `/client/stop` | 停止运行中的实例 |
| 设置实例通知地址 | `/client/set_notify_url` | 配置接收通知的回调地址 |
| 设置实例桥接ID | `/client/set_bridge_id` | 设置实例桥接标识 |
| 设置实例代理 | `/client/set_proxy` | 配置网络代理 |
| 升级实例 | `/client/upgrade` | 升级实例版本 |
| 获取实例状态 | `/client/get_status` | 查询实例当前状态 |
...
| 接口名称 | 端点 | 说明 |
|---------|------|------|
| 获取登录二维码 | `/login/get_qrcode` | 生成登录二维码 |
| 检查登录二维码状态 | `/login/check_qrcode` | 轮询二维码扫描状态 |
| 推送手机确认登录通知 | `/login/push_confirm` | 推送登录确认通知 |
| 验证登录 | `/login/verify` | 完成登录验证 |
...
| 接口名称 | 端点 | 说明 |
|---------|------|------|
| 获取登录账号信息 | `/user/get_info` | 获取当前登录用户详细信息 |
| 获取个人二维码 | `/user/get_qrcode` | 获取个人微信二维码 |
| 退出登录 | `/user/logout` | 退出当前登录 |

---

### 4. 联系人管理 (Contact) - 11个接口

**功能**: 管理微信好友和联系人

| 接口名称 | 端点 | 主要功能 |
|---------|------|----------|
| 同步联系人 | `/contact/sync` | 同步所有联系人信息 |
| 置顶聊天 | `/contact/top_chat` | 将聊天置顶 |
| 获取联系人详细 | `/contact/get_detail` | 获取单个联系人详细信息 |
| 批量获取联系人简要信息 | `/contact/batch_get_brief` | 批量获取联系人基本信息 |
| 通过手机号/微信号/QQ搜索 | `/contact/search` | 搜索微信用户 |
| 添加微信用户 | `/contact/add` | 添加好友 |
| 检查好友状态/同意好友申请 | `/contact/verify` | 处理好友请求 |
| 修改好友备注 | `/contact/set_remark` | 修改好友备注名 |
| 删除好友 | `/contact/delete` | 删除好友关系 |
...
**总计约 140 个 API 接口**,为微信自动化、社群运营、客服机器人、营销工具等应用场景提供了坚实的技术基础。
